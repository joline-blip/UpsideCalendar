...
async function purgeAllStaff(formData: FormData) {
  ...
  // - Bookings reference staff with onDelete: Restrict
  // - Availability blocks reference staff with onDelete: Cascade but we’ll delete explicitly
  // - Sessions reference staff with onDelete: Cascade but we’ll delete explicitly
  // - Audit logs may reference actorUserId (SetNull), but delete staff-authored actions for cleanliness
  const result = await prisma.$transaction(async (tx) => {
    const deletedBookings = await tx.booking.deleteMany({ where: { staffUserId: { in: staffIds } } });
    const deletedAvailability = await tx.availabilityBlock.deleteMany({ where: { userId: { in: staffIds } } });
    const deletedSessions = await tx.session.deleteMany({ where: { userId: { in: staffIds } } });
